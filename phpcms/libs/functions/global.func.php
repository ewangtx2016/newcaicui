...
function pageurl($urlrule, $page, $array = array()) {
	...
	if(strpos($url,'.html'))
	{
		...
		$url_first=explode('.html',$url_split[0]);
		if(strpos($url_first[0],'-'))
		{
			$url_second=explode('-',$url_first[0]);
			$url=$url_second[0].'-'.$url_split[1].'.html';
		}
		else
		{
			$url=$url_first[0].'-'.$url_split[1].'.html';
		}
	}
