...
class xwzx extends admin{
	...
	public function create() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				showmessage('操作成功','?m=xwzx&c=xwzx&a=create');
			}
			else{
				showmessage('操作失败，请重新提交','?m=xwzx&c=xwzx&a=create');
			}
		}
		else{
			$where['id']=3;
	...
	public function create2() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			$res=$this->lxwmset->update($data,$where);
			if($res){
				showmessage('操作成功','?m=xwzx&c=xwzx&a=create2');
			}
			else{
				showmessage('操作失败，请重新提交','?m=xwzx&c=xwzx&a=create2');
			}
		}
		else{
			$where['id']=27;
	...
	public function index() {
		$where='1=1';
		$where.=' and `lei`="企业动态" and `status_del`=0';
		//搜索
	...
	public function add() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				showmessage('操作成功','?m=xwzx&c=xwzx&a=index');
			}
			else{
				showmessage('操作失败，请重新提交','?m=xwzx&c=xwzx&a=add');
			}
		}
		else{
	...
	public function edit() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				...
				showmessage('操作失败，请重新提交','?m=xwzx&c=xwzx&a=edit');
			}
		}
		else{
			$where['id']=$_GET['id'];
	...
	//回收站
	public function xwzx_hsz() {
		$where='status_del =1 and `lei`="企业动态"';
		//搜索
		if(!empty($_GET['title'])) {
			$where .= " AND `title` like '%".$_GET['title']."%'";
		}
		...
		$datas = $this->db->listinfo($where, $order, $page, 20);
		$pages = $this->db->pages;
		include $this->admin_tpl('xwzx_hsz');
	}
	
	//还原
	public function xwzx_hszhy() {
		if(!empty($_GET['id'])){
			$where['id']=$_GET['id'];
			$data['status_del']=0;
			$datas = $this->db->update($data,$where);
			if($datas){
				showmessage('操作成功','?m=xwzx&c=xwzx&a=xwzx_hsz');
			}
			else{
				showmessage('操作失败，请重新提交','?m=xwzx&c=xwzx&a=xwzx_hsz');
			}
		}
		else{
			showmessage('操作失败，请重新提交111','?m=xwzx&c=xwzx&a=xwzx_hsz');	
		}	
	}
	
	//回收站删除
	public function xwzx_hszsc() {
		$userid=param::get_cookie('userid');
		//单条删除
		if(!empty($_GET['id'])){
			$where['id']=$_GET['id'];
