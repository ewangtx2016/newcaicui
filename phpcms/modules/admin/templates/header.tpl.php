...
<div class="subnav">
    ...
    <?php if(count($big_menu) == count($big_menu, 1)){?>
        ...
        <?php }else{?>
        <?php foreach($big_menu as $v) { ?>
            <?php if(isset($v)) { echo '<a class="add fb" href="'.$v[0].'"><em>'.$v[1].'</em></a>　';} else {$v = '';} ?>
            <?php //echo admin::submenu($_GET['menuid'],$v); ?>
        <?php }}?>
    </div>
</div>
<?php } ?>
