...
$(function(){
	...
	$("#title").formValidator({onshow: "必填",onfocus: "请输入标题"}).inputValidator({min:1,max:88,onerror:"不能超过88个字"}).ajaxValidator({
		type : "get",
		url : "",
		data :"m=set&c=set_yw&a=get_title&hiddenid="+$('#hidden_id').val(),
		datatype : "html",
		async:'false',
	...
	$("#one").formValidator({onshow:"必填",onfocus:"请输入图片文字"}).inputValidator({min:1,onerror:"不能为空"});
	$("#english").formValidator({onshow:"必填",onfocus:"请输入图片英文"}).inputValidator({min:1,onerror:"不能为空"});
});
//-->
</script>
<div class="pad-10">
<div class="common-form">
<form name="myform" id="myform" action="index.php?m=set&c=set_yw&a=add" method="post" enctype="multipart/form-data">
<fieldset>
	<legend>添加中博业务</legend>
<table class="table_form" width=100%>
    <tr><input type="hidden" name="info[lei]" id="lei" value="中博业务">
    	<td>图片</td>
        <td>
        	<a class="btn" id="btn">上传图片</a> 633px * 367px,最大800kb，支持jpg，gif，png格式。
    ...
    <tr>
        <td><span style="color:#f00;">*</span> 图片文字</td>
        <td><input type="text" style="width:600px;" name="info[one]" id="one" value="" /></td>
    </tr>
    <tr>
        <td><span style="color:#f00;">*</span> 图片英文</td>
