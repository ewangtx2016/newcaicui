...
class gyzb_cxjy extends admin{
	
	private $db;
	public function __construct() {
		$this->db = pc_base::load_model('gyzb_cxjy_model');
	}
	...
	public function create() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				showmessage('操作成功','?m=gyzb&c=gyzb_cxjy&a=create');
			}
			else{
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_cxjy&a=create');
			}
		}
		else{
			$where['id']=1;
	...
	public function create2() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			$res=$this->lxwmset->update($data,$where);
			if($res){
				showmessage('操作成功','?m=gyzb&c=gyzb_cxjy&a=create2');
			}
			else{
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_cxjy&a=create2');
			}
		}
		else{
			$where['id']=21;
	...
	public function add() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				showmessage('操作成功','?m=gyzb&c=gyzb_cxjy&a=index');
			}
			else{
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_cxjy&a=add');
			}
		}
		else{
	...
	public function edit() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				...
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_cxjy&a=edit');
			}
		}
		else{
			$where['id']=$_GET['id'];
	...
	public function cxjy_hsz() {
		...
		$datas = $this->db->listinfo($where, $order, $page, 20);
		$pages = $this->db->pages;
		include $this->admin_tpl('cxjy_hsz');
	}
	
	//还原
	public function cxjy_hszhy() {
		if(!empty($_GET['id'])){
			$where['id']=$_GET['id'];
			$data['status_del']=0;
			$datas = $this->db->update($data,$where);
			if($datas){
				showmessage('操作成功','?m=gyzb&c=gyzb_cxjy&a=cxjy_hsz');
			}
			else{
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_cxjy&a=cxjy_hsz');
			}
		}
		else{
			showmessage('操作失败，请重新提交111','?m=gyzb&c=gyzb_cxjy&a=cxjy_hsz');	
		}	
	}
	
	//回收站删除
	public function cxjy_hszsc() {
		$userid=param::get_cookie('userid');
		//单条删除
		if(!empty($_GET['id'])){
			$where['id']=$_GET['id'];
