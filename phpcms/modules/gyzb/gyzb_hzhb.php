...
class gyzb_hzhb extends admin{
	...
	public function create() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				showmessage('操作成功','?m=gyzb&c=gyzb_hzhb&a=create');
			}
			else{
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_hzhb&a=create');
			}
		}
		else{
			$where['id']=20;
			$res=$this->lxwmset->get_one($where);
			include $this->admin_tpl('createhzhb');
		}
	}
	...
	public function add() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				showmessage('操作成功','?m=gyzb&c=gyzb_hzhb&a=index');
			}
			else{
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_hzhb&a=add');
			}
		}
		else{
			include $this->admin_tpl('hzhbadd');
		}
	}
	...
	public function edit() {
		...
		if(!empty($_POST['dosubmit'])){
			...
			if($res){
				...
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_hzhb&a=edit');
			}
		}
		else{
			$where['id']=$_GET['id'];
			$res=$this->db->get_one($where);
			include $this->admin_tpl('hzhbedit');
		}
	}
	...
	public function hzhb_hsz() {
		...
		$datas = $this->db->listinfo($where, $order, $page, 20);
		$pages = $this->db->pages;
		include $this->admin_tpl('hzhb_hsz');
	}
	
	//还原
	public function hzhb_hszhy() {
		if(!empty($_GET['id'])){
			$where['id']=$_GET['id'];
			$data['status_del']=0;
			$datas = $this->db->update($data,$where);
			if($datas){
				showmessage('操作成功','?m=gyzb&c=gyzb_hzhb&a=hzhb_hsz');
			}
			else{
				showmessage('操作失败，请重新提交','?m=gyzb&c=gyzb_hzhb&a=hzhb_hsz');
			}
		}
		else{
			showmessage('操作失败，请重新提交111','?m=gyzb&c=gyzb_hzhb&a=hzhb_hsz');	
		}	
	}
	
	//回收站删除
	public function hzhb_hszsc() {
		$userid=param::get_cookie('userid');
		//单条删除
		if(!empty($_GET['id'])){
			$where['id']=$_GET['id'];
