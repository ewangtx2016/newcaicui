...
$(function(){
	...
	$("#company").formValidator({onshow: "必填",onfocus: "请输入名称"}).inputValidator({min:1,max:88,onerror:"不能超过88个字"}).ajaxValidator({
		type : "get",
		url : "",
...
<form name="myform" id="myform" action="index.php?m=gyzb&c=gyzb_fb&a=edit" method="post" enctype="multipart/form-data">
<fieldset>
	<legend>编辑分部联系方式</legend>
<table class="table_form" width=100%>
    ...
    <tr>
        <td> 传真</td>
        <td><input type="text" style="width:600px;" name="info[fax]" id="fax" value="<?php echo $res['fax']?>" /></td>
    </tr>
    ...
    <tr>
    	<td> 城市</td>
        <td><?php echo menu_linkage(1,'city',$res['city']);?></td>
    </tr>
	...
	var uploader = new plupload.Uploader({//创建实例的构造方法
		...
		init: {
			...
			FileUploaded: function(up, file, info) { //文件上传成功的时候触发
				...
				$("#" + file.id).html("<img src='" + data.pic + "'/><div><a class='delImg' imgurl='"+data.pic+"'>删除</a></div><input type='hidden' name='info[com_img]' value='"+data.pic+"'>");
			},
